...
{
    printk(KERN_WARNING "[monitor] SOFT LIMIT %s pid=%d rss=%ld limit=%lu\n",
           id, pid, rss, limit);
}
                         ...
                         unsigned long limit, long rss)
{
    ...
    printk(KERN_WARNING "[monitor] HARD LIMIT %s pid=%d rss=%ld limit=%lu\n",
           id, pid, rss, limit);
}
...
static long monitor_ioctl(struct file *f, unsigned int cmd, unsigned long arg)
{
    ...
    if (cmd == MONITOR_REGISTER) {
        ...
        printk(KERN_INFO "[monitor] Registered PID=%d soft=%lu hard=%lu\n",
               entry->pid, entry->soft_limit, entry->hard_limit);
